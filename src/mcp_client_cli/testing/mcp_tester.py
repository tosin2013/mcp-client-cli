...
class MCPServerTester:
    ...
    async def test_server_connectivity(self, server_config: ServerConfig, server_name: str) -> TestResult:
        """
    ...
    async def test_tool_discovery(self, server_config: ServerConfig, server_name: str) -> TestResult:
        """
        ...
        try:
            ...
            if not toolkit:
                ...
                conn_result = await self.test_server_connectivity(server_config, server_name)
    ...
    async def test_tool_execution(self, server_config: ServerConfig, server_name: str, tool_name: str, test_args: Dict[str, Any] = None) -> TestResult:
        """
            ...
            if not config.mcp_servers:
                ...
                for server_name, server_config in config.mcp_servers.items():
                    if not server_config.command:
                        issues.append(f"Server '{server_name}' has no command specified")
    ...
    async def run_comprehensive_test_suite(self, server_name: Optional[str] = None) -> Dict[str, TestSuite]:
        """
        ...
        for name, server_config in servers_to_test.items():
            ...
            if conn_result.status == TestStatus.PASSED:
                ...
                if discovery_result.status == TestStatus.PASSED and discovery_result.details.get("tool_count", 0) > 0:
                    tool_names = discovery_result.details.get("tool_names", [])
                    # Test first tool as sample (can be extended for comprehensive testing)
                    if tool_names:
                        execution_result = await self.test_tool_execution(server_config, name, tool_names[0])
            ...
            if test_results:
                overall_confidence = sum(r.confidence_score for r in test_results) / len(test_results)
            else:
