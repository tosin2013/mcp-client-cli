...
class PerformanceMetrics:
    ...
    def calculate_derived_metrics(self, test_duration: float):
        ...
        self.success_rate = self.success_count / total_requests if total_requests > 0 else 0.0
        self.throughput_rps = total_requests / test_duration if test_duration > 0 else 0.0
...
class MCPPerformanceTester:
    ...
    async def benchmark_tool_execution(self, server_config: ServerConfig, server_name: str) -> TestResult:
        """
        ...
        try:
            ...
            for tool_name in tools[:5]:  # Limit to first 5 tools for reasonable test time
            ...
            if overall_metrics.avg_response_time > self.config.max_response_time_ms:
                issues.append(f"Average response time ({overall_metrics.avg_response_time:.1f}ms) exceeds threshold")
            
            if overall_metrics.success_rate < self.config.min_success_rate:
                issues.append(f"Success rate ({overall_metrics.success_rate:.2%}) below threshold")
    ...
    async def test_concurrent_connections(self, server_config: ServerConfig, server_name: str) -> TestResult:
        """
    ...
    async def measure_response_times(self, server_config: ServerConfig, server_name: str) -> TestResult:
        """
        ...
        try:
            ...
            if all_response_times:
                ...
                if overall_metrics.p95_response_time > self.config.max_response_time_ms:
                    issues.append(f"P95 response time ({overall_metrics.p95_response_time:.1f}ms) exceeds threshold")
                
                if overall_metrics.max_response_time > self.config.max_response_time_ms * 2:
                    issues.append(f"Maximum response time ({overall_metrics.max_response_time:.1f}ms) is excessive")
    ...
    async def test_resource_usage(self, server_config: ServerConfig, server_name: str) -> TestResult:
        """
        ...
        try:
            ...
            if resource_data["peak_memory_mb"] > self.config.max_memory_usage_mb:
                issues.append(f"Peak memory usage ({resource_data['peak_memory_mb']:.1f}MB) exceeds threshold")
            
            if resource_data["avg_cpu_percent"] > self.config.max_cpu_usage_percent:
                issues.append(f"Average CPU usage ({resource_data['avg_cpu_percent']:.1f}%) exceeds threshold")
            ...
            if memory_trend > 0.1:  # More than 10% increase over time
                issues.append(f"Potential memory leak detected (trend: +{memory_trend:.1%})")
    ...
    async def run_comprehensive_performance_suite(self, server_config: ServerConfig, server_name: str) -> Dict[str, TestResult]:
        """
        ...
        if self.config.test_response_times:
            results["response_times"] = await self.measure_response_times(server_config, server_name)
        
        if self.config.test_concurrent_connections:
            results["concurrent_connections"] = await self.test_concurrent_connections(server_config, server_name)
        
        if self.config.test_resource_usage:
            results["resource_usage"] = await self.test_resource_usage(server_config, server_name)
        
        # Tool benchmarking
        results["tool_benchmark"] = await self.benchmark_tool_execution(server_config, server_name)
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        avg_score = sum(grade_scores.get(grade, 0) for grade in grades) / len(grades) if grades else 0
    ...
    async def _benchmark_single_tool(self, toolkit: McpToolkit, tool_name: str, iterations: int = 10) -> PerformanceMetrics:
    ...
    async def _run_load_test(self, server_config: ServerConfig, server_name: str, 
        ...
        await asyncio.gather(*[toolkit.initialize() for toolkit in toolkits], return_exceptions=True)
        ...
        await asyncio.gather(*[worker(toolkit, i) for i, toolkit in enumerate(toolkits)], return_exceptions=True)
    ...
    async def _measure_scenario_response_times(self, toolkit: McpToolkit, tool_name: str, 
                                             scenario_name: str, call_count: int) -> PerformanceMetrics:
        ...
        if scenario_name == "cold_start":
            ...
            await asyncio.gather(*[single_call() for _ in range(call_count)], return_exceptions=True)
    ...
    def _analyze_memory_trend(self, memory_samples: List[float]) -> float:
        ...
        numerator = sum((x - x_mean) * (y - y_mean) for x, y in zip(x_values, memory_samples))
