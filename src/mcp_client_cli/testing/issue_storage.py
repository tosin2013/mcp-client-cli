...
from .issue_detector import Issue, IssueType, IssueSeverity, HealthMetrics
...
class IssueTrackingManager:
    ...
    async def _init_db(self, db) -> None:
        ...
        await db.execute("CREATE INDEX IF NOT EXISTS idx_issues_server_name ON issues (server_name)")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_issues_timestamp ON issues (timestamp)")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_issues_type ON issues (issue_type)")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_remediation_issue_id ON remediation_results (issue_id)")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_health_metrics_server ON health_metrics (server_name)")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_health_metrics_timestamp ON health_metrics (timestamp)")
    ...
    async def save_health_metrics(self, metrics: HealthMetrics) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            """, (
                ...
                metrics.last_successful_connection.isoformat() if metrics.last_successful_connection else None,
                metrics.consecutive_failures
            ))
            
            await db.commit()
    
    async def get_issues(self, server_name: Optional[str] = None,
    ...
    async def get_remediation_results(self, issue_id: Optional[str] = None,
    ...
    async def get_health_metrics(self, server_name: Optional[str] = None,
                               since: Optional[datetime] = None,
                               limit: int = 100) -> List[HealthMetrics]:
        """
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            for row in rows:
                metrics = HealthMetrics(
                    ...
                    last_successful_connection=datetime.fromisoformat(row[9]) if row[9] else None,
    ...
    async def get_issue_statistics(self, server_name: Optional[str] = None,
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Total issues
            async with db.execute(f"SELECT COUNT(*) FROM issues {where_clause}", params) as cursor:
            ...
            remediation_where = where_clause.replace("issues", "remediation_results r JOIN issues i ON r.issue_id = i.id")
            ...
            remediation_success_rate = successful_remediations / total_remediations if total_remediations > 0 else 0.0
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if existing:
                ...
                await db.execute("""
                    ...
                    WHERE id = ?
                """, (new_occurrence_count, datetime.now().isoformat(), new_success_rate, pattern_id))
            else:
                # Create new pattern
                ...
                """, (
                    pattern_id, pattern_signature, issue.issue_type.value, 1,
    ...
    async def get_pattern_insights(self, server_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            return {
                "most_common_patterns": [{"issue_type": row[0], "occurrences": row[1]} for row in common_patterns],
                "successful_patterns": [{"issue_type": row[0], "success_rate": row[1], "pattern_count": row[2]} for row in successful_patterns],
                "problematic_patterns": [{"signature": row[0], "issue_type": row[1], "occurrences": row[2], "success_rate": row[3]} for row in problematic_patterns]
            }
