...
from .mcp_tester import TestResult, TestSuite, TestStatus
...
class TestResultManager:
    ...
    async def get_test_suite(self, suite_id: str) -> Optional[TestSuite]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            for row in result_rows:
                test_name, status, confidence_score, execution_time, message, details_json, error_info, timestamp = row
            ...
            server_name, total_tests, passed_tests, failed_tests, error_tests, skipped_tests, overall_confidence, execution_time, timestamp = suite_row
    ...
    async def get_latest_test_suites(self, server_name: Optional[str] = None, limit: int = 10) -> List[TestSuite]:
        """
    ...
    async def get_test_statistics(self, server_name: Optional[str] = None, days: int = 30) -> Dict[str, Any]:
        """
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            total_suites, avg_confidence, avg_execution_time, total_passed, total_failed, total_errors, total_tests = stats_row
            ...
            if not server_name:
                ...
                for row in server_rows:
                    server, suite_count, server_confidence, server_passed, server_total = row
                    server_success_rate = (server_passed / server_total) if server_total > 0 else 0.0
