...
class TestMCPSecurityTester:
    ...
    async def test_authentication_test_success(self, security_tester, server_config):
        ...
        with patch.object(security_tester, '_test_no_credentials', return_value={"secure": True}), \
             patch.object(security_tester, '_test_invalid_credentials', return_value={"secure": True}), \
             patch.object(security_tester, '_test_expired_credentials', return_value={"secure": True}), \
             patch.object(security_tester, '_test_malformed_auth_headers', return_value={"secure": True}):
            
            result = await security_tester.test_authentication(server_config, "test_server")
    ...
    async def test_authentication_test_failure(self, security_tester, server_config):
        ...
        with patch.object(security_tester, '_test_no_credentials', return_value={
            "secure": False, 
            ...
            assert security_tester._vulnerabilities[0].vulnerability_type == "authentication_bypass"
    ...
    async def test_authentication_test_error(self, security_tester, server_config):
        ...
        with patch.object(security_tester, '_test_no_credentials', side_effect=Exception("Test error")):
            result = await security_tester.test_authentication(server_config, "test_server")
    ...
    async def test_authorization_test(self, security_tester, server_config):
        ...
        with patch('mcp_client_cli.testing.security_tester.McpToolkit', return_value=mock_toolkit), \
             patch.object(security_tester, '_test_privilege_escalation', return_value={"secure": True}), \
             patch.object(security_tester, '_test_unauthorized_tool_access', return_value={"secure": True}), \
             patch.object(security_tester, '_test_resource_access_controls', return_value={"secure": True}):
            
            result = await security_tester.test_authorization(server_config, "test_server")
    ...
    async def test_input_validation_test(self, security_tester, server_config):
        ...
        with patch('mcp_client_cli.testing.security_tester.McpToolkit', return_value=mock_toolkit), \
             patch.object(security_tester, '_test_tool_with_payload', return_value={"vulnerable": False}):
            
            result = await security_tester.test_input_validation(server_config, "test_server")
    ...
    async def test_data_sanitization_test(self, security_tester, server_config):
        ...
        with patch.object(security_tester, '_test_xss_prevention', return_value={"secure": True}), \
             patch.object(security_tester, '_test_sql_injection_prevention', return_value={"secure": True}), \
             patch.object(security_tester, '_test_command_injection_prevention', return_value={"secure": True}), \
             patch.object(security_tester, '_test_path_traversal_prevention', return_value={"secure": True}):
            
            result = await security_tester.test_data_sanitization(server_config, "test_server")
    ...
    async def test_comprehensive_security_scan(self, security_tester, server_config):
        ...
        with patch.object(security_tester, 'test_authentication') as mock_auth, \
             ...
             patch.object(security_tester, 'test_data_sanitization') as mock_sanitization:
            ...
            results = await security_tester.run_comprehensive_security_scan(server_config, "test_server")
    ...
    async def test_test_tool_with_payload_vulnerable(self, security_tester):
        ...
        mock_toolkit = Mock()
        mock_toolkit.call_tool = AsyncMock(return_value="Payload: <script>alert('XSS')</script>")
    ...
    def test_generate_security_recommendations(self, security_tester):
        ...
        security_tester._vulnerabilities = [
            SecurityVulnerability("authentication_bypass", "high", "Auth bypass", "auth"),
            SecurityVulnerability("input_validation_bypass", "medium", "Input bypass", "input"),
        ]
