...
class TestMCPPerformanceTester:
    ...
    @pytest.mark.asyncio
    async def test_benchmark_tool_execution_success(self, performance_tester, server_config):
        ...
        with patch('mcp_client_cli.testing.performance_tester.McpToolkit', return_value=mock_toolkit), \
             patch.object(performance_tester, '_benchmark_single_tool', return_value=mock_metrics):
            
            result = await performance_tester.benchmark_tool_execution(server_config, "test_server")
    ...
    @pytest.mark.asyncio
    async def test_benchmark_tool_execution_no_tools(self, performance_tester, server_config):
    ...
    @pytest.mark.asyncio
    async def test_benchmark_tool_execution_error(self, performance_tester, server_config):
        """Test tool benchmarking with error."""
        with patch('mcp_client_cli.testing.performance_tester.McpToolkit', side_effect=Exception("Test error")):
    ...
    async def test_test_concurrent_connections(self, performance_tester, server_config):
        ...
        async def mock_load_test(server_config, server_name, concurrent_users, duration_seconds):
            metrics = PerformanceMetrics()
        ...
        result = await performance_tester.test_concurrent_connections(server_config, "test_server")
    ...
    async def test_measure_response_times(self, performance_tester, server_config):
        ...
        with patch('mcp_client_cli.testing.performance_tester.McpToolkit', return_value=mock_toolkit), \
             patch.object(performance_tester, '_measure_scenario_response_times', return_value=mock_metrics):
            
            result = await performance_tester.measure_response_times(server_config, "test_server")
    ...
    @pytest.mark.asyncio
    async def test_measure_response_times_no_tools(self, performance_tester, server_config):
    ...
    async def test_test_resource_usage(self, performance_tester, server_config):
        ...
        with patch('mcp_client_cli.testing.performance_tester.McpToolkit', return_value=mock_toolkit), \
             patch('mcp_client_cli.testing.performance_tester.ResourceMonitor') as mock_monitor_class:
            ...
            result = await performance_tester.test_resource_usage(server_config, "test_server")
    ...
    @pytest.mark.asyncio
    async def test_run_comprehensive_performance_suite(self, performance_tester, server_config):
        """Test comprehensive performance suite."""
        # Mock all individual test methods
        with patch.object(performance_tester, 'measure_response_times') as mock_response, \
             patch.object(performance_tester, 'test_concurrent_connections') as mock_concurrent, \
             patch.object(performance_tester, 'test_resource_usage') as mock_resource, \
             patch.object(performance_tester, 'benchmark_tool_execution') as mock_benchmark:
            ...
            results = await performance_tester.run_comprehensive_performance_suite(server_config, "test_server")
    ...
    async def test_benchmark_single_tool(self, performance_tester):
        ...
        metrics = await performance_tester._benchmark_single_tool(mock_toolkit, "test_tool", iterations=3)
    ...
    def test_detect_bottlenecks(self, performance_tester):
        ...
        metrics.p95_response_time = 10000.0  # High variability (>3x avg response time: 10000 > 9000)
    ...
    def test_analyze_memory_trend(self, performance_tester):
        ...
        increasing_samples = [100.0, 110.0, 120.0, 130.0, 140.0, 150.0, 160.0, 170.0, 180.0, 190.0]
        trend = performance_tester._analyze_memory_trend(increasing_samples)
        assert trend > 0.1  # Should detect significant increase
        
        # Test stable trend
        stable_samples = [100.0, 102.0, 98.0, 101.0, 99.0, 100.0, 103.0, 97.0, 101.0, 100.0]
...
class TestResourceMonitor:
    ...
    async def test_resource_monitor_lifecycle(self):
        ...
        with patch('mcp_client_cli.testing.performance_tester.psutil.Process') as mock_process_class:
            mock_process = Mock()
