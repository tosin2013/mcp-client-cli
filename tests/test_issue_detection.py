...
from src.mcp_client_cli.testing.issue_detector import (
    MCPIssueDetector, Issue, IssueType, IssueSeverity, HealthMetrics, IssuePattern
)
from src.mcp_client_cli.testing.remediation import (
    MCPRemediationEngine, RemediationResult, RemediationStatus, RemediationStrategy, RetryConfig
)
...
class TestMCPIssueDetector:
    ...
    async def test_monitor_server_health_success(self, issue_detector, server_config):
        ...
        with patch('src.mcp_client_cli.testing.issue_detector.McpToolkit') as mock_toolkit:
            ...
            metrics = await issue_detector.monitor_server_health(server_config, "test_server")
    ...
    async def test_monitor_server_health_failure(self, issue_detector, server_config):
        ...
        with patch('src.mcp_client_cli.testing.issue_detector.McpToolkit') as mock_toolkit:
            mock_instance = AsyncMock()
            mock_instance._start_session = AsyncMock(side_effect=Exception("Connection failed"))
            mock_instance.close = AsyncMock()
            mock_toolkit.return_value = mock_instance
    ...
    @pytest.mark.asyncio
    async def test_analyze_test_failures_connection_error(self, issue_detector, test_result_connection_failure):
        """Test analysis of connection failure."""
        issues = await issue_detector.analyze_test_failures(test_result_connection_failure)
    ...
    @pytest.mark.asyncio
    async def test_analyze_test_failures_timeout(self, issue_detector, test_result_timeout):
        ...
        assert issue.confidence_score > 0.5
        assert "timed out" in issue.error_message.lower() or "timeout" in issue.error_message.lower()
...
class TestMCPRemediationEngine:
    ...
    @pytest.mark.asyncio
    async def test_remediate_issue_success(self, remediation_engine, connection_issue, server_config):
        """Test successful issue remediation."""
        with patch.object(remediation_engine, '_test_server_connectivity', return_value=True):
            result = await remediation_engine.remediate_issue(connection_issue, server_config)
            
            assert isinstance(result, RemediationResult)
            assert result.issue_id == connection_issue.issue_id
            assert result.status in [RemediationStatus.SUCCESS, RemediationStatus.PARTIAL_SUCCESS]
    ...
    @pytest.mark.asyncio
    async def test_retry_strategy(self, remediation_engine, connection_issue, server_config):
        ...
        with patch.object(remediation_engine, '_test_server_connectivity', side_effect=connectivity_results):
            # Get retry action
            actions = remediation_engine._get_remediation_actions(connection_issue)
            retry_action = next((a for a in actions if a.strategy == RemediationStrategy.RETRY), None)
            assert retry_action is not None
            
            result = await remediation_engine._execute_retry_strategy(retry_action, connection_issue, server_config)
    ...
    async def test_dependency_install_strategy(self, remediation_engine, server_config):
        ...
        with patch.object(remediation_engine, '_test_server_connectivity', return_value=True):
            result = await remediation_engine.remediate_issue(dependency_issue, server_config)
    ...
    def test_get_success_rate(self, remediation_engine):
        ...
        # Add test results to history
        results = [
            RemediationResult("a1", "i1", RemediationStatus.SUCCESS, 0.9, 1.0, "Success"),
            RemediationResult("a2", "i2", RemediationStatus.SUCCESS, 0.8, 2.0, "Success"),
...
class TestIssueTrackingManager:
    ...
    @pytest.mark.asyncio
    async def test_save_and_get_remediation_result(self, issue_manager, test_issue, test_remediation_result):
    ...
    @pytest.mark.asyncio
    async def test_save_and_get_health_metrics(self, issue_manager, test_health_metrics):
        ...
        assert retrieved_metrics.connection_success_rate == test_health_metrics.connection_success_rate
    ...
    async def test_resolve_issue(self, issue_manager, test_issue):
        ...
        # Resolve issue
        success = await issue_manager.resolve_issue(test_issue.issue_id, "Fixed by manual intervention")
        assert success
    ...
    @pytest.mark.asyncio
    async def test_get_issue_statistics(self, issue_manager, test_issue, test_remediation_result):
    ...
    async def test_learn_from_pattern(self, issue_manager, test_issue):
        ...
        assert any(pattern["occurrences"] >= 2 for pattern in insights["most_common_patterns"])
    ...
    async def test_filtering_and_pagination(self, issue_manager):
        ...
        for i in range(5):
            issue = Issue(
                issue_id=f"issue_{i}",
                issue_type=IssueType.CONNECTION_FAILURE if i % 2 == 0 else IssueType.TIMEOUT,
        ...
        connection_issues = await issue_manager.get_issues(issue_type=IssueType.CONNECTION_FAILURE)
        assert len(connection_issues) == 3  # Issues 0, 2, 4
        
        # Test severity filtering
        high_severity_issues = await issue_manager.get_issues(severity=IssueSeverity.HIGH)
